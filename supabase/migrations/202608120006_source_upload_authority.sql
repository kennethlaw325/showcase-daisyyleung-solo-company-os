-- Make the signed-ticket route the only authority that can reserve and
-- finalize private source uploads. Browser sessions keep read access but can
-- no longer forge Storage objects or extracted prompt text directly.
drop policy if exists source_documents_member_write on storage.objects;

revoke all on function public.register_source_item(uuid, uuid, text, text, text, bigint, text) from public, anon, authenticated, service_role;
revoke all on function public.update_source_extraction(uuid, text, text, text, text) from public, anon, authenticated, service_role;
